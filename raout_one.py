...
def get_india_time():
    ...
    utc_time = datetime.fromtimestamp(
        current_time, tz=pytz.utc)  # Convert to UTC time
...
class RouteOne(QWidget):
    def __init__(self):
        ...
        self.detector = VehicleDetection1(
            video_path='videos/vid1.mp4', db_name='cam1.db')
        ...
        # Updates based on the camera's FPS
        self.timer.start(1000 / self.detector.fps)
        ...
        # Add space between the two widgets (adjust as needed)
        horizontal_layout.setSpacing(25)
        ...
        self.live_feed_entry_label.setStyleSheet(
            "background-color: #eaeff5; border: 1px dashed #ccc;")
        camera_layout.addWidget(self.live_feed_entry_label, 0, 0)
        grid_widget = QWidget()
        grid_widget.setStyleSheet(
            "QWidget { border: 1px solid #333; border-radius: 15px; background-color: #f9f9f9; }")
        ...
        # 75% width for the camera view (3 parts)
        ...
        self.snapshot_image_label.setStyleSheet(
            "background-color: #eaeff5; border: 1px solid #ccc;")
        ...
        # 25% width for the vehicle details (1 part)
        ...
        self.table.setHorizontalHeaderLabels(
            ["Entry Counter", "Entry Time", "Exit Counter", "Exit Time", "Plate Number", "Snapshot"])
        layout.addWidget(self.table)

        # Set font for column headers
        # Set font family, size, and weight for headers
        ...
        # Column 0 resizes to content
        header.setSectionResizeMode(0, QHeaderView.ResizeToContents)
        header.setSectionResizeMode(
            1, QHeaderView.Stretch)           # Column 1 stretches
        # Column 2 resizes to content
        header.setSectionResizeMode(2, QHeaderView.ResizeToContents)
        header.setSectionResizeMode(
            3, QHeaderView.Stretch)           # Column 3 stretches
        ...
        self.line_y_slider.valueChanged.connect(
            lambda value: self.update_settings(value, self.offset_slider.value()))
        self.offset_slider.valueChanged.connect(
            lambda value: self.update_settings(self.line_y_slider.value(), value))
    ...
    def update_video_feed(self):
        ...
        if ret:
            ...
            print(
                f"Entry Counter: {self.detector.entry_counter}, Exit Counter: {self.detector.exit_counter}")
            height, width, _ = frame.shape
            byte_array = frame.tobytes()
            qimg = QImage(byte_array, width, height, QImage.Format_BGR888)
            pixmap = QPixmap.fromImage(qimg)
            self.live_feed_entry_label.setPixmap(pixmap.scaled(
                self.live_feed_entry_label.size(), Qt.KeepAspectRatio))
            self.total_entries_label.setText(
                f"Total Entries\n{self.detector.entry_counter}")
            self.total_exits_label.setText(
            ...
            if self.detector.entry_counter > self.last_entry_counter:
                ...
                self.add_vehicle_to_table(
                    self.detector.vehicle_ids, current_time, "Entry")
    ...
    def add_vehicle_to_table(self, vehicle_ids, current_time, entry_or_exit):
        if vehicle_ids:
            # Get the last vehicle ID (Plate Number)
            last_vehicle_id = list(vehicle_ids.keys())[-1]
            ...
            if vehicle_snapshot:
                ...
                # Set snapshot in the last column
                self.table.setCellWidget(row_position, 5, snapshot_label)
                ...
                self.table.setColumnWidth(
                    5, max(pixmap.width(), self.table.columnWidth(5)))
                # Set QSizePolicy for better resizing behavior
                snapshot_label.setSizePolicy(
    ...
    def display_snapshot(self, snapshot_blob):
        if snapshot_blob:
            ...
            pixmap = QPixmap.fromImage(image)
            self.snapshot_image_label.setPixmap(pixmap.scaled(
                self.snapshot_image_label.size(), Qt.KeepAspectRatio))

    def change_entry_camera(self, event):
        dialog = CameraSelectionDialog(self)
        if dialog.exec() == QDialog.Accepted:
            selected_camera = dialog.get_selected_camera()
            # Update vehicle detection camera
            self.detector.update_video_source(selected_camera)
    ...
    def assign_camera(self, camera_type, camera_index):
        ...
        if camera_type == "entry":
            # Update vehicle detection source
            self.detector.update_video_source(camera_index)
            # Optional: keep local reference
            self.entry_cap = cv2.VideoCapture(camera_index)
    ...
    def update_line_y(self):
        ...
        self.settings_manager.save_settings(
            new_value, self.offset_slider.value(), self.detector.camera_index)
    ...
    def update_offset(self):
        ...
        # Save the updated settings to file
        self.settings_manager.save_settings(
            self.line_y_slider.value(), new_value, self.detector.camera_index)
    ...
    def start_video_processing(self):
        ...
        self.video_capture = cv2.VideoCapture(0)  # or your video source
        # ...existing code...
    ...
    def process_frame(self, frame):
        try:
            ...
            # Force cleanup of any temporary arrays
            gc.collect()
